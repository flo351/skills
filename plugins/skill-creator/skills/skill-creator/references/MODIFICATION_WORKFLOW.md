...
Read the config (`~/.claude/skill-creator/config.json`) to fetch known `repo_dir` paths:
- `repo_dir` (stable, e.g. `$HOME/code/<stable-name>`)
- `beta_repo_dir` (beta, e.g. `$HOME/code/<beta-name>`)
